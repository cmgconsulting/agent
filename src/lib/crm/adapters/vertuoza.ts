...
export class VertuozaAdapter implements CRMAdapter {
  type = 'vertuoza' as const
  private apiKey: string

  constructor(credentials: Record<string, string>) {
    this.apiKey = credentials.api_key
    if (!this.apiKey) throw new Error('Cle API Vertuoza manquante')
  }

  async testConnection(): Promise<{ success: boolean; error?: string }> {
    return { success: false, error: 'Integration Vertuoza en cours de developpement' }
  }
