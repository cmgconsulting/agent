...
export class ToltekAdapter implements CRMAdapter {
  type = 'toltek' as const
  private apiKey: string

  constructor(credentials: Record<string, string>) {
    this.apiKey = credentials.api_key
    if (!this.apiKey) throw new Error('Cle API Toltek manquante')
  }

  async testConnection(): Promise<{ success: boolean; error?: string }> {
    return { success: false, error: 'Integration Toltek en cours de developpement' }
  }
