...
export class GraneetAdapter implements CRMAdapter {
  type = 'graneet' as const
  private apiKey: string

  constructor(credentials: Record<string, string>) {
    this.apiKey = credentials.api_key
    if (!this.apiKey) throw new Error('Cle API Graneet manquante')
  }

  async testConnection(): Promise<{ success: boolean; error?: string }> {
    return { success: false, error: 'Integration Graneet en cours de developpement' }
  }
