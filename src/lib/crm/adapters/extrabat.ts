...
export class ExtrabatAdapter implements CRMAdapter {
  type = 'extrabat' as const
  ...
  constructor(credentials: Record<string, string>) {
    ...
    if (!this.apiKey) throw new Error('Cle API Extrabat manquante')
  }

  async testConnection(): Promise<{ success: boolean; error?: string }> {
    return { success: false, error: 'Integration Extrabat en cours de developpement' }
  }
