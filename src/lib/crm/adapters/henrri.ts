...
export class HenrriAdapter implements CRMAdapter {
  type = 'henrri' as const
  private apiKey: string

  constructor(credentials: Record<string, string>) {
    this.apiKey = credentials.api_key
    if (!this.apiKey) throw new Error('Cle API Henrri manquante')
  }

  async testConnection(): Promise<{ success: boolean; error?: string }> {
    return { success: false, error: 'Integration Henrri en cours de developpement' }
  }
