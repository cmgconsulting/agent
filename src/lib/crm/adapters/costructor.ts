...
export class CostructorAdapter implements CRMAdapter {
  type = 'costructor' as const
  ...
  constructor(credentials: Record<string, string>) {
    ...
    if (!this.apiKey) throw new Error('Cle API Costructor manquante')
  }

  async testConnection(): Promise<{ success: boolean; error?: string }> {
    return { success: false, error: 'Integration Costructor en cours de developpement' }
  }
